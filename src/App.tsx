...
Fibre2Fashion TexPro RM Historical Pricing Streamlit Dashboard
Optimized for embedding directly inside Google Sites via standard iframe layouts.
...
st.set_page_config(
    page_title="Fibre2Fashion Raw Material Pricing",
    layout="wide",
...
st.title("📊 Raw Material Pricing Dashboard")
st.caption("Historical price trends pulled from Fibre2Fashion TexPro API")

# 2. MATERIAL ID MAPPING
# =========================================================================
# REPLACE THESE PLACEHOLDER PRODUCT IDS WITH THE PROPRIETARY VENDOR IDS
# RECEIVED FROM YOUR FIBRE2FASHION TEXPRO INTEGRATION TEAM.
# =========================================================================
MATERIAL_MAPPING = {
    "ZMAC": 103,      # Zamac Alloy Product ID
    "PU": 104,        # Polyurethane Product ID
    "Rubber": 105,    # Rubber Product ID
    "Cotton": 101,    # Cotton Product ID
    "Spandex": 102,   # Spandex Product ID
    "RPES": 106,      # Recycled Polyester Product ID
    "PES": 107,       # Virgin Polyester Product ID
    "Paper": 108,     # Paper Product ID
    "PP": 109         # Polypropylene Product ID
}
...
# Secure password mask for bearer token
...
# 4. FETCH DATA ACTION
if st.sidebar.button("Fetch Historical Prices", type="primary"):
    if not api_token:
        st.warning("⚠️ Please provide your Fibre2Fashion Bearer Token to fetch live data.")
    elif not selected_materials:
        st.warning("⚠️ Please choose at least one material option.")
    elif start_date > end_date:
        st.error("❌ Start date cannot reside in the future relative to the End date.")
    else:
        with st.spinner("Querrying Fibre2Fashion TexPro servers..."):
            # Map selection list to vendor product ID arrays
            mapped_ids = [MATERIAL_MAPPING[name] for name in selected_materials]
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    raw_rows = api_json.get("Data", []) or api_json.get("data", [])
                    
                    if not raw_rows and isinstance(api_json, list):
                        raw_rows = api_json
                    ...
                    if raw_rows:
                        ...
                        # Harmonize naming variations
                        if "Date" in raw_df.columns:
                            raw_df["date"] = pd.to_datetime(raw_df["Date"])
                        elif "date" in raw_df.columns:
                            raw_df["date"] = pd.to_datetime(raw_df["date"])
                            
                        # Reshape to wide format for Plotly Express plotting
                        # We pivot the table so dates are row keys, material names are columns
                        # Let's map IDs back to human names if ProductName is missing
                        id_to_name = {v: k for k, v in MATERIAL_MAPPING.items()}
                        
                        if "ProductId" in raw_df.columns and "ProductName" not in raw_df.columns:
                            raw_df["ProductName"] = raw_df["ProductId"].map(id_to_name)
                        
                        # Plotly chart drawing
                        st.subheader("📊 Price Visualizer (USD/KG)")
                        
                        # Line Chart with seamless transparent design
                        fig = px.line(
                            raw_df,
                            x="date",
                            y="Price" if "Price" in raw_df.columns else "price",
                            color="ProductName" if "ProductName" in raw_df.columns else "product_name",
                            markers=True,
                            title="Fibre2Fashion Price Tracking",
                        ...
                        # CSV Exporter
                        csv_file = raw_df.to_csv(index=False).encode('utf-8')
                        st.download_button(
                            label="📥 Export Table as CSV",
                            data=csv_file,
                            file_name=f"fibre2fashion_export_{start_date}.csv",
                            mime="text/csv"
                        )
                    else:
                        st.warning("⚠️ Request succeeded but no historical data is returned for this period.")
                        st.write("Raw API Output:", api_json)
                else:
                    st.error(f"❌ API Proxy returned status {response.status_code}")
                    st.text(response.text)
            except Exception as e:
                st.error(f"❌ Connection error failed: {str(e)}")
else:
    st.info("👈 Enter your Bearer Token and click 'Fetch Historical Prices' to populate the visualizer.")
`;
